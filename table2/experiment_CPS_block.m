function [] = experiment_CPS_block(seed)
rng(seed)
addpath('../functions') 
%Data(Blocking)
...
[n,d] = size(X);
g_ID = findgroups(zipcode);%numel(unique(g_ID))/numel(Y)
...
save(['CPS\results_experiment_CPS_' num2str(seed) '.mat'],'error_est','r_square','K_n');
end