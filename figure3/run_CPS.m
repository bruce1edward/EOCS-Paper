...
error_est(i,j,5) = norm(beta_DA  - beta_oracle)/norm(beta_oracle);
...
r_square(i,j,5) = 1 - norm(Y - X*beta_DA)^2/norm(Y- mean(Y))^2;
r_square(i,j,6) = 1 - norm(Y - X*beta_LL)^2/norm(Y- mean(Y))^2;
r_square(i,j,7) = 1 - norm(Y - X*beta_C)^2/norm(Y- mean(Y))^2;
end
end
